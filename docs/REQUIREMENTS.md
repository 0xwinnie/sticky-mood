...
## 2e. 第五轮决议（2026-09-17）— 存储架构改为内部 flash 为主

借测第一轮暴露：SD 与墨水屏共用 SPI2 会互相干扰，SD 可靠性尚未验证。
但机器**没插 SD 也能正常开机使用**（已实测），而一条心情记录极小（mood +
energy + intention + 时间戳 + 转写文字，约 0.5 KB）。因此把核心存储从 SD 挪到内部 flash。

| # | 决议 | 备注 |
|---|---|---|
| D13 | **内部 flash = 主力存储；SD = 可选（仅原始录音 / 导出备份）** | 32 MB flash 中约 30 MB 原本闲置。新增 4 MB `storage` 分区（FATFS + 磨损均衡，挂载在 `/data`，见 `partitions.csv` + `main/board/storage.cpp`）。核心心情记录不依赖 SD，**没卡也能记、绝不丢**。SD 与内部同为 FATFS，导出 = 普通文件拷贝 |
| D14 | **保存对用户无感：成功不提示，失败才用局部刷新提示** | 墨水屏双稳态——确认页刷出后画面静止，写库藏在静止画面背后，用户看不到卡顿。故砍掉原"存档中… 已存档 ✓"的多余文案，默认自动保存。仅当写入失败时，局部刷新弹一行失败提示（成功路径零打扰） |
| D15 | **4 MB 满了由用户手动导出 + 清理，不自动滚动覆盖** | 4 MB ≈ 8000 条 ≈ 每天 3 条用 7 年，远超"保留三个月"需求，正常情况下不会满。满了不自动删旧数据（避免静默丢失），而是提示用户导出后自行清理 |

> 注：D13 取代了 §4 与 D4/D5 中"存 SD 卡"的旧表述；下面这些章节保留作历史记录，
> 以本节为准。原 littlefs 方案因需联网拉组件（本机 GitHub 传输不稳）改用 IDF 自带的 FATFS。
