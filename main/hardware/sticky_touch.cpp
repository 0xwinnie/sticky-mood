...
bool StickyTouch::init()
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(kTag, "Touch I2C0 init failed: %s", esp_err_to_name(err));
        return false;
    }
    ...
    ESP_LOGI(kTag,
             "GT911 ready: sensor=%ux%u logical=%ux%u address=0x%02X",
             sensor_width,
             sensor_height,
             kLogicalWidth,
             kLogicalHeight,
...
TouchEvent StickyTouch::read_touch()
{
    ...
    if (count > 0) {
        // GT911 reports its 480x800 sensor already mapped into an 800x480 range.
        // Combine that swap with the display's 270-degree rotation so this layer
        // returns portrait coordinates that line up with what StickyDisplay draws.
        const uint16_t logical_x = scale_coordinate(point.x, kNativeWidth, kLogicalWidth - 1);
        const uint16_t mapped_y = std::min<uint16_t>(point.y, kNativeHeight);
        const uint16_t logical_y =
            scale_coordinate(kNativeHeight - mapped_y, kNativeHeight, kLogicalHeight - 1);
